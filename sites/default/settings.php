 ...
 * @endcode
 */
//Adding this so Varnish will do it's job
$conf['image_allow_insecure_derivatives'] = TRUE;
$conf['disable_riot_player'] = false;
$conf['disable_riot_chat'] = false;
...
if (!isset($_SERVER['PANTHEON_ENVIRONMENT'])) {
  ...
  $conf['site_name'] = 'SR LOCAL';
	$conf['disable_riot_player'] = false;
	$conf['disable_riot_chat'] = true;
	...
	$conf['search_active_modules'] = array('node');
	
} else {
	switch($_SERVER['PANTHEON_ENVIRONMENT']) {
		case 'dev':
  		$conf['site_name'] = 'SR DEV';
			$conf['css_gzip_compression'] = FALSE;
			$conf['js_gzip_compression'] = FALSE;
			$conf['cache'] = 0;
			$conf['cache_lifetime'] = 0;
		...
		case 'test':
  		...
  		$conf['site_name'] = 'SR TEST';
			...
			break;
		case 'live':
			$base_url = 'http://streamriot.com';
			$conf['googleanalytics_account'] = 'UA-40669121-1';
			# Use Redis for caching.
			$conf['redis_client_interface'] = 'PhpRedis';
			$conf['cache_backends'][] = 'sites/all/modules/redis/redis.autoload.inc';
			$conf['cache_default_class'] = 'Redis_Cache';
			...
			// Remove WWW.
			if ($_SERVER['HTTP_HOST'] == 'www.streamriot.com' ||
					$_SERVER['HTTP_HOST'] == 'live-streamriot.gotpantheon.com') {
				header('HTTP/1.0 301 Moved Permanently');
				header('Location: http://streamriot.com'. $_SERVER['REQUEST_URI']);
				exit();
			}
			break;	
	}
 ...
 * for you.
 */
 #$base_url = 'http://streamriot.com';  // NO trailing slash!
